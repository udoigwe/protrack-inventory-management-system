				...
				success: function (response) {
					if (response.error == false) {
						...
						editModal
							.find("#logo")
							.attr({
								src: `${API_HOST_NAME}/logos/${store.store_logo}`,
						...
						editModal
							.find("#store_address1")
							.val(store.store_address);
						editModal
							.find("#store_status")
							.selectpicker("val", store.store_status);
		...
		swal({
			...
			if (result.value) {
				...
				if ($.inArray(extension, validFileExtensions) == -1) {
					...
					showSimpleMessage(
						"Attention",
						"Store logo must be a jpeg, jpg, png file",
						"error"
					);
					return false;
				}
				...
				if ($("#store_logo").get(0).files[0].size > 1024 * 1000) {
					...
					showSimpleMessage(
						"Attention",
						"Store Logo must not be more than 1MB in size",
						"error"
					);
					return false;
				}
			...
			if (result.value) {
				...
				if (storeLogo) {
					...
					if ($.inArray(extension, validFileExtensions) == -1) {
						...
						showSimpleMessage(
							"Attention",
							"Store Logo must be a jpeg, jpg, png format",
							"error"
						);
						return false;
					}
			...
			columns: [
				...
				},
				{
					...
					render: function (data, type, row, meta) {
						var actions =
							`
                            <a href="javascript:void(0);" class="btn btn-link font-18 text-muted btn-sm btn-edit" title="Edit Store" data-id="` +
							data +
                            ...
                            <a href="javascript:void(0);" class="btn btn-link font-18 text-muted btn-sm btn-delete" title="Delete Store" data-id="` +
							data +
